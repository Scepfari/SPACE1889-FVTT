export default class TurnMarker
{
	...
	setAnimation()
	{
		...
		function Animate() 
		{
			if (turnMarker.sprite._destroyed || !turnMarker.sprite) 
			{
				canvas.app.ticker.remove(Animate);
				if (!turnMarker.sprite.reallyDestroy) new turnMarker.TM_Class();
			}
			else
			{
				if (turnMarker.container.visible)
					turnMarker.sprite.rotation += 0.01 * turnMarker.speed;
			}
		}
